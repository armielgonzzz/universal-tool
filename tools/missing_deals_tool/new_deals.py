...
def add_constant_columns(cm_db_final_df: pd.DataFrame) -> pd.DataFrame:
    ...
    cm_db_final_df['Deal - Deal Status'] = cm_db_final_df['Deal Status'] if 'Deal Status' in cm_db_final_df.columns else ''
    cm_db_final_df['Person - Timezone'] = ''
    cm_db_final_df['Deal - Marketing Medium'] = 'Text'
    cm_db_final_df['Deal - Deal Stage'] = cm_db_final_df['Stage'] if 'Deal Status' in cm_db_final_df.columns else 'Deals to Process'
    cm_db_final_df['Deal - Non-voice Qualification Channel'] = cm_db_final_df['Non-voice Qualification Channel'] if 'Non-voice Qualification Channel' in cm_db_final_df.columns else ''
    cm_db_final_df['Deal - Reason for Not Selling'] = cm_db_final_df['Reason for Not Selling'] if 'Reason for Not Selling' in cm_db_final_df.columns else ''
    cm_db_final_df['Deal - Contact Confirmation'] = cm_db_final_df['Contact Confirmation'] if 'Contact Confirmation' in cm_db_final_df.columns else ''
    cm_db_final_df['Notes'] = ''
    cm_db_final_df['Assigned to user'] = 'Keena'
    cm_db_final_df['Done'] = 'Done'
    cm_db_final_df['Type'] = 'Text'
    cm_db_final_df['Deal ID'] = ''
    cm_db_final_df['Note Content'] = ''
    cm_db_final_df['Activity note'] = cm_db_final_df['Text']
    cm_db_final_df['Deal - Deal creation date'] = cm_db_final_df['Deal Created Date']
    cm_db_final_df['Subject'] = cm_db_final_df.apply(
        lambda x: f"Text from {x['Phone']} to {x['To']}",
        axis=1
    )
...
def no_result_constants(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Person - Phone'] = df['From']
    df['Person - Phone 1'] = df['From']
    df['Deal - Label'] = ''
    ...
    df['Notes'] = ''
    df['Assigned to user'] = 'Keena'
    df['Done'] = 'Done'
    df['Type'] = 'Text'
    df['Deal ID'] = ''
    df['Note Content'] = ''
    df['Note (if any)'] = 'No Result'
    df['Deal - Title'] = df.apply(lambda x: f"No Name {x['From']}", axis=1)
    df['Person - Name'] = df.apply(lambda x: f"No Name {x['From']}", axis=1)
    df['Deal - Deal Status'] = df['Deal Status'] if 'Deal Status' in df.columns else ''
    df['Deal - Deal Stage'] = df['Stage'] if 'Deal Status' in df.columns else 'Deals to Process'
    df['Deal - Non-voice Qualification Channel'] = df['Non-voice Qualification Channel'] if 'Non-voice Qualification Channel' in df.columns else ''
    df['Deal - Reason for Not Selling'] = df['Reason for Not Selling'] if 'Reason for Not Selling' in df.columns else ''
    df['Deal - Contact Confirmation'] = df['Contact Confirmation'] if 'Contact Confirmation' in df.columns else ''
    df['Activity note'] = df['Text']
    df['Deal - Deal creation date'] = df['Deal Created Date']
    df['Subject'] = df.apply(
...
def get_timezone_dict() -> dict:
    ...
    timezone_df = pd.read_csv("C:/Users/armie/work/cm_tools/universal_tool/data/tz_file/Time Zones.csv", low_memory=False)
...
def process_new_deals(no_deal_id_final: pd.DataFrame,
    ...
    if not no_deal_id_final.empty:
        no_deal_id_final.rename(columns={'From': 'Phone'}, inplace=True)
        warnings.filterwarnings("ignore", category=DeprecationWarning)
        no_deal_id_final['id'] = no_deal_id_final['id']
        if 'Non-voice Qualification Channel' in no_deal_id_final.columns:
    ...
    final_new_deals_df['Person - Timezone'] = final_new_deals_df.apply(get_timezone,
                                                                    ...
                                                                    axis=1)
    new_deals_export_df = final_new_deals_df[output_columns] # .sort_values(by=['Activity creation date'])
